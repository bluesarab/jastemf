 ...
 */
package org.jastemf.sfple.semantics;
...
public class DeweyAddress implements Comparable<DeweyAddress>, Cloneable {
	...
	/**
	 * Constructs a new <tt>DeweyAddress</tt>, not yet initialized with
	 * any value.
	 */
	public DeweyAddress() {
	 ...
	 * if <i>smaller</i> than the other one, both addresses are equal or this
	...
	/**
	 * Constructs a new <tt>DeweyAddress</tt> object from a string, i.e.
	 * tries to parse the string as a valid address.
	 * @param address The string to interpret as <i>Dewey</i> address.
	 * @return A <tt>DeweyAddress</tt> object representing the parsed address.
	 * @throws IllegalArgumentException Is thrown, if the given string is no
	 * valid <i>Dewey</i> address.
	 */
	public static DeweyAddress fromString(String address)
	throws IllegalArgumentException {
		...
		for (i = 0; i < length; i++) {
			...
			while (Character.isDigit(address.charAt(i)) && i < length) {
				digit.append(address.charAt(i));
				i++;
			}
			result.addAddressPart(Integer.parseInt(digit.toString()));
			if (address.charAt(i) != '.')
				break;
			i++; // Skip the "." separator
		}
		if (length == 0 || i != length ||
				!Character.isDigit(address.charAt(0)) ||
				!Character.isDigit(address.charAt(length - 1)))
			throw new IllegalArgumentException("The string ["+
					address +"] is no valid Dewey address.");
		return result;
	}
}
