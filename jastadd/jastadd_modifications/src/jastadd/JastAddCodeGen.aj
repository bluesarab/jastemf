...
aspect JastAddCodeGen {
				...
				for (int j = 0; j < getNumTypeDecl(); j++) {
					if (getTypeDecl(j) instanceof ASTDecl) {
						...
						if (dest.implementsInterface(name)) {
							...
							// dest.name());
							for (Iterator iter = d.getClassBodyDecls(); iter
									.hasNext();) {
								ClassBodyObject o = (ClassBodyObject) iter
										.next();
								if (o.node instanceof ASTAspectMethodDeclaration
									...
									if (!dest.hasClassBodyDecl(o.signature()))
										dest.classBodyDecls
												.add(new ClassBodyObject(
														o.node/* .fullCopy() */,
														o.fileName, o.line,
														o.getAspectName()));
								} else if (o.node instanceof ASTAspectRefineMethodDeclaration) {
									ClassBodyObject object = new ClassBodyObject(
											o.node/* .fullCopy() */, o.fileName,
							...
							for (Iterator iter = d.refinedClassBodyDecls
							...
							for (int k = 0; k < d.getNumSynDecl(); k++) {
								// System.out.println("    adding syndecl " +
								// d.getSynDecl(k).signature());
								dest.addSynDecl((SynDecl) d.getSynDecl(k)
										.fullCopy());
							}

							for (int k = 0; k < d.getNumSynEq(); k++) {
								// System.out.println("    adding syneq " +
								// d.getSynEq(k).signature());
								dest.addSynEq((SynEq) d.getSynEq(k).fullCopy());
							}

							for (int k = 0; k < d.getNumInhDecl(); k++) {
								// System.out.println("    adding inhdecl " +
								// d.getInhDecl(k).signature());
								dest.addInhDecl((InhDecl) d.getInhDecl(k)
										.fullCopy());
							}

							for (int k = 0; k < d.getNumInhEq(); k++) {
								// System.out.println("    adding inheq " +
								// d.getInhEq(k).signature());
								dest.addInhEq((InhEq) d.getInhEq(k).fullCopy());
							}
							for (int k = 0; k < d.getNumCollDecl(); k++) {
								dest.addCollDecl((CollDecl) d.getCollDecl(k)
										.fullCopy());
							}
						}
					}
	...
	public void Grammar.jastAddGen(File outputDir, String grammarName,
			String packageName, boolean publicModifier) {
		...
		for (int i = 0; i < getNumTypeDecl(); i++) {
			getTypeDecl(i).jastAddGen(outputDir, grammarName, packageName,
					publicModifier);
		}
	}
	...
	public String ClassDecl.typeDeclarationString() {
		...
		if (s.equals(""))
			return modifiers() + "class " + getIdDecl().getID()
					+ typeParameters + " extends " + extendsName + " {";
		else
			return modifiers() + "class " + getIdDecl().getID()
					+ typeParameters + " extends " + extendsName
					+ " implements " + s + " {";
	...
	public String InterfaceDecl.typeDeclarationString() {
		String s = interfacesString();
		if (s.equals(""))
			return modifiers() + "interface " + getIdDecl().getID()
					+ typeParameters + " {";
		else
			return modifiers() + "interface " + getIdDecl().getID()
					+ typeParameters + " extends " + s + " {";
	}
	...
	public void TypeDecl.jastAddGen(File outputDir, String grammarName,
			String packageName, boolean publicModifier) {
	}

	public void InterfaceDecl.jastAddGen(File outputDir, String grammarName,
			String packageName, boolean publicModifier) {
		File file = null;
		try {
			if (packageName != null && !packageName.equals("")) {
				file = new File(outputDir, packageName.replace('.',
						File.separatorChar) + File.separator + name() + ".java");
			} else {
			...
			/*
			 * StringBuffer buf = new StringBuffer(); for(Iterator iter =
			 * getClassBodyDecls(); iter.hasNext(); ) { ClassBodyObject o =
			 * (ClassBodyObject)iter.next(); jrag.AST.SimpleNode n = o.node;
			 * 
			 * if(!(n instanceof ASTAspectMethodDeclaration) && !(n instanceof
			 * ASTAspectFieldDeclaration)) { buf.append("    // Declared in " +
			 * o.fileName + " at line " + o.line + "\n");
	...
	public void ClassDecl.jastAddGen(File outputDir, String grammarName,
			String packageName, boolean publicModifier) {
		File file = null;
		try {
			if (packageName != null && !packageName.equals("")) {
			...
			for (Iterator iter = getClassBodyDecls(); iter.hasNext();) {
				...
				buf.append("    // Declared in " + o.fileName + " at line "
						+ o.line + "\n");
				if (!o.comments.equals(""))
					buf.append(o.comments + " ");
				n.unparseClassBodyDeclaration(buf, name(), false); // Fix
																	// AspectJ
				buf.append("\n\n");
			}
			stream.println(buf.toString());
	...
	public void ASTDecl.jastAddGen(File outputDir, String grammarName,
			String packageName, boolean publicModifier) {
		File file = null;
		try {
			if (packageName != null && !packageName.equals("")) {
				...
				file = new File(outputDir, implName() + ".java");
			}
			PrintStream stream = new PrintStream(new FileOutputStream(file));
			if (license != null)
				stream.println(license);
			...
			if (ASTNode.beaver && isBuiltInNode(name())) {
				...
				if (ASTNode.isBuiltInList(name())
						|| ASTNode.isBuiltInOpt(name()))
					name = name + "<T>";
			...
			/*
			 * // Now done before refinements jjtGen(new
			 * java.io.PrintWriter(stream), grammarName, JastAdd.jjtree,
			 * JastAdd.rewriteEnabled); int j = 0; for(Iterator iter =
			 * getComponents(); iter.hasNext(); ) { Components c =
			 * (Components)iter.next(); c.jaddGen(stream, j, publicModifier,
			 * name()); if(!(c instanceof TokenComponent)) { j++; } }
			 */
