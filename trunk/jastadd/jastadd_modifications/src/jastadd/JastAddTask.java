...
public class JastAddTask extends Task {
	...
	public void setVisitCheck(boolean b) {
		novisitcheck = !b;
	}
	...
	// generate strongly connected component optimization for circular
	// attributes
	...
	public void execute() throws BuildException {
		...
		if (packageName != null) {
			name.append(packageName.replace('.', File.separatorChar)
					+ File.separator);
		}
		...
		if (generated.exists()) {
			...
			for (Iterator iter = files.iterator(); iter.hasNext();) {
				...
				if (!file.exists()
						|| file.lastModified() > generated.lastModified())
					changed = true;
			}
			if (!changed) {
				return;
		...
		if (defaultMap != null)
			args.add("--defaultMap=" + defaultMap);
		if (defaultSet != null)
			args.add("--defaultSet=" + defaultSet);
		if (lazyMaps)
			args.add("--lazyMaps");
